...
export function buildSummaryCards(summary = {}, withReach = false) {
  ...
  const cards = [
    ...
    { label: 'Conv', value: formatInt(summary.tracked_conversions || summary.conversions || 0) },
  ...
  if (summary.cpa) cards.push({ label: 'CPA', value: `${formatMoney(summary.cpa)} ${currency}` })
  if (summary.roas) cards.push({ label: 'ROAS', value: `${Number(summary.roas || 0).toFixed(2)}x` })
