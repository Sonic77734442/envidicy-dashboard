...
function mapConnection(row, importedCount = 0) {
  return {
    ...
    refresh_token: row.refresh_token_encrypted || null,
...
export async function saveProviderConnectionTokenDb(agencyId, provider, tokenPayload = {}) {
  const connection = await getProviderConnectionDb(agencyId, provider)
  if (!connection) return null

  const scopes = Array.isArray(tokenPayload.scopes)
    ? tokenPayload.scopes
  ...
  await query(
    `
      ...
      set status = 'connected',
          ...
          token_last_refreshed_at = case when $3 is not null then now() else token_last_refreshed_at end,
          external_owner = coalesce($7, external_owner),
          updated_at = now()
      where id = $1 and agency_id = $2
    `,
    [
      ...
      scopes,
      tokenExpiresAt,
      tokenPayload.external_owner || null,
    ]
  )
...
export async function listExternalAccountsDb(agencyId) {
  ...
  return result.rows.map((row) => ({
    ...
    selected_for_sync: Boolean(row.selected_for_sync),
    ...
    const ext = await query(
      `
        insert into external_accounts (
          agency_id, connection_id, provider, external_id, name, currency, account_status, selected_for_sync, metadata_json
        )
      ...
      [
        ...
        Boolean(account.selected_for_sync),
...
export async function setExternalAccountsSelectedForSyncDb(agencyId, provider, externalAccountIds = []) {
  const ids = Array.from(new Set((externalAccountIds || []).map(String)))
  await query(
    `
      update external_accounts
      set selected_for_sync = case when id = any($3::uuid[]) then true else false end,
          updated_at = now()
      where agency_id = $1
        and provider = $2
    `,
    [agencyId, provider, ids]
  )
  return listExternalAccountsDb(agencyId)
}

export async function replaceGoogleAdsCustomerLinksDb(agencyId, connectionId, discoveredCustomers = []) {
  await query('delete from google_ads_customer_links where agency_id = $1 and connection_id = $2', [agencyId, connectionId])

  for (const customer of discoveredCustomers || []) {
    await query(
      `
        insert into google_ads_customer_links (
          agency_id,
          connection_id,
          parent_customer_id,
          customer_id,
      ...
      [
        ...
        Boolean(customer.is_manager),
        Number(customer.level || 0),
        Boolean(customer.hidden),
        Boolean(customer.test_account),
...
export async function activateSelectedExternalAccountsDb(agencyId, provider) {
  await query(
    `
      update dashboard_accounts da
      set status = case when ea.selected_for_sync then 'active' else 'archived' end,
          updated_at = now()
      from external_accounts ea
      where da.external_account_id = ea.id
        and da.agency_id = $1
        and da.provider = $2
        and ea.agency_id = $1
        and ea.provider = $2
