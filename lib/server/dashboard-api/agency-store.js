...
import {
  ...
  replaceGoogleAdsCustomerLinksDb,
  saveMetaConnectionTokenDb,
  saveProviderConnectionTokenDb,
  setExternalAccountsSelectedForSyncDb,
  activateSelectedExternalAccountsDb,
...
export async function saveProviderConnectionTokenForAgency(agencyId, provider, tokenPayload = {}) {
  if (hasDatabase()) return saveProviderConnectionTokenDb(agencyId, provider, tokenPayload)
  if (provider === 'meta') return completeMetaOAuthForAgency(agencyId, tokenPayload)
  return connectProvider(agencyId, provider)
}
...
export async function setExternalAccountsSelectedForSyncForAgency(agencyId, provider, externalAccountIds = []) {
  if (hasDatabase()) return setExternalAccountsSelectedForSyncDb(agencyId, provider, externalAccountIds)
  return listExternalAccounts(agencyId)
}

export async function replaceGoogleAdsCustomerLinksForAgency(agencyId, connectionId, discoveredCustomers = []) {
  if (hasDatabase()) return replaceGoogleAdsCustomerLinksDb(agencyId, connectionId, discoveredCustomers)
  return null
}

export async function activateSelectedExternalAccountsForAgency(agencyId, provider) {
  if (hasDatabase()) return activateSelectedExternalAccountsDb(agencyId, provider)
  return listDashboardAccounts(agencyId)
}
