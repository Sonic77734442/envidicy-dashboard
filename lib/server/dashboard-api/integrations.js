import {
  ...
  replaceGoogleAdsCustomerLinksForAgency,
  saveProviderConnectionTokenForAgency,
  setExternalAccountsSelectedForSyncForAgency,
  activateSelectedExternalAccountsForAgency,
...
export async function finishGoogleOAuthForAgency(agencyId, tokenPayload) {
  return saveProviderConnectionTokenForAgency(agencyId, 'google', tokenPayload)
}
...
export async function updateSelectedExternalAccounts(agencyId, provider, accountIds) {
  const externalAccounts = await setExternalAccountsSelectedForSyncForAgency(agencyId, provider, accountIds)
  return {
...
export async function replaceImportedAccounts(agencyId, connectionId, importedAccounts = []) {
  const imported = await replaceImportedAccountsForAgencyConnection(agencyId, connectionId, importedAccounts)
  return {
    imported,
    ...(await getIntegrationsSnapshot(agencyId)),
  }
...
export async function persistGoogleCustomerGraph(agencyId, connectionId, discoveredCustomers = []) {
  await replaceGoogleAdsCustomerLinksForAgency(agencyId, connectionId, discoveredCustomers)
  return getIntegrationsSnapshot(agencyId)
}

export async function activateSelectedExternalAccounts(agencyId, provider) {
  const dashboardAccounts = await activateSelectedExternalAccountsForAgency(agencyId, provider)
  return {
    dashboard_accounts: dashboardAccounts,
    ...(await getIntegrationsSnapshot(agencyId)),
  }
